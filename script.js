/**
 * DevDNA v2 - Massive Upgrade Main Logic
 * - Fixed download watermark & icon
 * - Fixed quiz grid (2x2 desktop)
 * - Fixed toast floating
 * - Fixed share link to https://devdna-2trh.onrender.com/
 * - New: SFX toggle, typing animation, boot screen, compatibility, breakdown,
 *        custom cursor, matrix rain, reveal sequence, total counter, QR on card,
 *        announcement + event lock + hash routing for admin/uptime/404
 */
...
// Compatibility exactly per spec
const COMPATIBILITY = {
...
const DOM = {
    ...
    totalCounterWrap: document.getElementById('total-users-counter'),
...
const SOUND_FILES = {
    ...
    error: 'audio/select.mp3' // fallback
};
...
function initSound() {
    ...
    Object.keys(SOUND_FILES).forEach(key => {
        ...
        a.addEventListener('error', () => {
            // Fail silently if files missing, fallback to silent
            console.warn(`[DevDNA] Audio ${key} not found at ${SOUND_FILES[key]} - using fallback`);
        });
    });
...
// Add hover SFX to all buttons dynamically
function bindHoverSFX() {
    document.addEventListener('mouseover', (e) => {
        const target = e.target.closest('button, .option-card');
        if (target) {
            // throttle via dataset
            const now = Date.now();
            const last = parseInt(target.dataset.lastHover || '0',10);
            if (now - last > 200) {
                playSFX('click');
                target.dataset.lastHover = String(now);
            }
        }
    }, { passive:true });
}
...
function initMatrixRain() {
    ...
    function draw() {
        ctx.fillStyle = 'rgba(10,10,15,0.07)'; // trail
        ...
        for (let i=0; i<cols; i++) {
            ...
            ctx.fillStyle = isPurple ? 'rgba(168,85,247,0.75)' : 'rgba(0,255,153,0.65)';

            ctx.fillText(char, x, y);

            if (y > h && Math.random() > 0.975) drops[i] = 0;
            drops[i] += 0.28 + Math.random()*0.6; // slow
        }
...
function initParticles() {
    ...
    function create(){
        ...
        particles=Array.from({length:count},()=>({
            ...
            r:Math.random()*1.7+0.3, hue: Math.random()>0.5?265:195, alpha:Math.random()*0.55+0.12
        }));
    }
    function draw(){
        ...
        for(let i=0;i<particles.length;i++){
            for(let j=i+1;j<particles.length;j++){
                ...
                if(dist<130){
                    ...
                    ctx.strokeStyle=`hsla(${particles[i].hue},100%,70%,${0.07*(1-dist/130)})`;
        ...
        particles.forEach(p=>{
            ...
            ctx.shadowBlur=10; ctx.shadowColor=`hsla(${p.hue},100%,70%,${p.alpha})`;
...
function initCustomCursor() {
    ...
    // hover effects
    document.addEventListener('mouseover', (e)=>{
        if(e.target.closest('button, .option-card, a, .chip')){
    ...
    window.addEventListener('mousemove', (e)=>{
        ...
        trail.animate([{transform:'translate(-50%,-50%) scale(1)', opacity:0.6},{transform:'translate(-50%,-50%) scale(0)', opacity:0}], {duration:400, easing:'ease-out'}).onfinish=()=>trail.remove();
    });
}

// ============================================================================
// BOOT SCREEN (Feature 3)
// ============================================================================
const BOOT_LINES = [
    '> Initializing DevDNA...',
...
function runBootScreen() {
    return new Promise(resolve=>{
        const container = DOM.bootLines;
        container.innerHTML='';
        BOOT_LINES.forEach((txt,i)=>{
            const div=document.createElement('div');
            div.className='boot-line';
            div.textContent='';
            div.dataset.full=txt;
            container.appendChild(div);
        });

        let lineIdx=0;
        function typeLine(idx){
            if(idx>=BOOT_LINES.length){ finishProgress(); return; }
            ...
            const interval=setInterval(()=>{
                ...
                charIdx++;
                if(charIdx>=full.length){ clearInterval(interval); setTimeout(()=>typeLine(idx+1), 180); }
            }, 38);
        }
        typeLine(0);
        ...
        function finishProgress(){
            setTimeout(()=>{
                ...
                setTimeout(()=>{ bootDone=true; resolve(); }, 750);
            }, 500);
        }
    });
}
...
async function runLandingTyping(){
    ...
    DOM.heroTitleTyped.textContent='';
    DOM.heroSubtitleTyped.textContent='';
    DOM.heroTitleCursor.style.display='inline-block';
    DOM.heroSubtitleCursor.style.display='none';

    await typeWriter(DOM.heroTitleTyped, title, 90);
    DOM.heroTitleCursor.style.display='none';
    DOM.heroSubtitleCursor.style.display='inline-block';
    await typeWriter(DOM.heroSubtitleTyped, subtitle, 42);
    // keep cursor blinking after
    setTimeout(()=>{ /* keep */ }, 200);
}
...
// ============================================================================
function startQuiz(){
    if(!currentSettings.eventLive){
        // if event closed, don't allow quiz
        showLocked();
        return;
    }
    playSFX('complete');
...
function calculateResult(){
    let max=-1, winner=ARCHETYPE_ORDER[0];
    ARCHETYPE_ORDER.forEach(k=>{ if(scores[k]>max){max=scores[k]; winner=k;} });
    return winner;
}

// ============================================================================
// RESULT REVEAL ANIMATION (Feature 8)
// ============================================================================
const REVEAL_LINES = [
...
function finishQuizReveal(){
    ...
    setTimeout(()=>{
        ...
        function showNext(){
            ...
            div.textContent=REVEAL_LINES[idx];
            DOM.revealLines.appendChild(div);
            setTimeout(()=>div.classList.add('show'), 60);
            // glitch last line
            if(idx===REVEAL_LINES.length-1){
                setTimeout(()=>div.classList.add('glitch'), 200);
            }
            idx++;
            setTimeout(showNext, idx===1?550:420);
        }
        showNext();
    }, 350);
}
...
function showResult(archetype){
    ...
    DOM.resultName.classList.remove('glow-in');
    void DOM.resultName.offsetWidth;
    DOM.resultName.classList.add('glow-in');
    ...
    archetype.traits.forEach(t=>{
        ...
        pill.textContent=t;
        pill.style.borderColor=`${archetype.color}44`;
...
function renderBreakdown(scoreObj){
    ...
    const entries=ARCHETYPE_ORDER.map(k=>({
        key:k,
        ...ARCHETYPES[k],
        count: scoreObj[k]||0,
        percent: Math.round(((scoreObj[k]||0)/total)*100)
    ...
    entries.forEach(item=>{
        ...
        div.innerHTML=`
            <div class="breakdown-top">
                <span class="breakdown-name" style="color:${item.color}">${item.emoji} ${item.name}</span>
                <span class="breakdown-percent" style="color:${item.color}">${item.percent}%</span>
            </div>
            <div class="breakdown-bar-track">
                <div class="breakdown-bar-fill" style="width:0%; background: linear-gradient(90deg, ${item.gradient[0]}, ${item.gradient[1]}); box-shadow:0 0 10px ${item.color}88;"></div>
            </div>
        `;
        DOM.breakdownList.appendChild(div);
        // animate width after insert
        requestAnimationFrame(()=>{
            const fill=div.querySelector('.breakdown-bar-fill');
            setTimeout(()=>fill.style.width=`${item.percent}%`, 80);
        });
    });
}
...
function setupLeaderboard(){
    ...
    leaderboardUnsub=subscribeToLeaderboard((counts)=>{
        ...
        // update total counter for landing
        totalDevelopers=counts.total || Object.keys(counts).filter(k=>k!=='total').reduce((s,k)=>s+(counts[k]||0),0);
    ...
    sorted.forEach(item=>{
        ...
        div.innerHTML=`
            <div class="leader-item-top">
                <div class="leader-icon" style="border-color:${item.color}33; box-shadow:0 0 10px ${item.color}22;">${item.emoji}</div>
                <div class="leader-info"><span class="leader-name">${item.name}</span><span class="leader-count">${item.count} developers</span></div>
            </div>
            <span class="leader-percent" style="color:${item.color}">${percent}%</span>
            <div class="leader-bar-wrap"><div class="leader-bar-fill" style="width:${width}%; background: linear-gradient(90deg, ${item.gradient[0]}, ${item.gradient[1]}); box-shadow:0 0 10px ${item.color}66;"></div></div>
        `;
...
function animateTotalCounter(target){
    ...
    function tick(){
        ...
        else { DOM.counterNum.textContent=target.toLocaleString(); }
    }
    tick();
}

// ============================================================================
// Canvas Export - FIX 1 (watermark perfectly centered, icon bright) + Feature 10 QR
// ============================================================================
...
function generateQRCodeDataURL(text){
    return new Promise((resolve,reject)=>{
        try{
            ...
            const qr=new QRCode(DOM.qrTemp, {
                text: text,
                width: 120,
                height: 120,
                colorDark: '#ffffff',
            ...
            // QRCode.js renders async, wait a bit
            setTimeout(()=>{
...
async function generateResultCard(){
    ...
    g1.addColorStop(0,'rgba(168,85,247,0.32)'); g1.addColorStop(1,'transparent');
    ctx.fillStyle=g1; ctx.fillRect(0,0,W,H);
    const g2=ctx.createRadialGradient(W,100,0,W,100,650);
    g2.addColorStop(0,'rgba(0,204,255,0.24)'); g2.addColorStop(1,'transparent');
    ...
    // FIX 1: Perfectly centered watermark
    ctx.save();
    ctx.globalAlpha=0.12; // 10-15%
    ctx.fillStyle='white';
    ctx.font='900 210px "Orbitron", sans-serif';
    ctx.textAlign='center';
    ctx.textBaseline='middle';
    // Perfect center
    ctx.fillText('DevDNA', W/2, H/2);
    ctx.restore();

    // Aura behind icon - brighter
    ctx.save();
    ctx.globalAlpha=0.55;
    ctx.filter='blur(42px)';
    ctx.fillStyle=arch.color;
    ctx.beginPath(); ctx.arc(W/2, 380, 145, 0, Math.PI*2); ctx.fill();
    ctx.restore();
    ctx.filter='none';

    // Brand top
    ctx.fillStyle='rgba(255,255,255,0.55)'; ctx.font='600 22px "JetBrains Mono", monospace';
    ctx.textAlign='center'; ctx.fillText('DevDNA', W/2, 110);
    ctx.fillStyle='rgba(161,161,181,0.75)'; ctx.font='500 16px "JetBrains Mono", monospace';
    ctx.fillText('ARCHETYPE DECODED // SYS.V1', W/2, 140);

    // Icon container - stronger neon glow
    ctx.save();
    ...
    if(ctx.roundRect){ ctx.roundRect(W/2-88,190,176,176,30); }
    else { ctx.rect(W/2-88,190,176,176); }
    ctx.fill(); ctx.stroke();
    ctx.restore();

    // Emoji - MUCH brighter and bigger
    ctx.save();
    ctx.shadowColor=arch.color;
    ctx.shadowBlur=28;
    ctx.font='92px serif';
    ...
    ctx.fillStyle='white';
    ctx.fillText(arch.emoji, W/2, 278);
    ...
    // Name
    ctx.fillStyle='white'; ctx.font='900 58px "Orbitron", sans-serif'; ctx.textAlign='center';
    ctx.fillText(arch.name.toUpperCase(), W/2, 470);

    const grad=ctx.createLinearGradient(W/2-90,0,W/2+90,0);
    grad.addColorStop(0, arch.gradient[0]); grad.addColorStop(1, arch.gradient[1]);
    ctx.fillStyle=grad; ctx.fillRect(W/2-80,494,160,4);

    // Description
    ctx.fillStyle='#c5c5d6'; ctx.font='400 26px "Space Grotesk", sans-serif'; ctx.textAlign='center';
    const afterDesc=wrapText(ctx, arch.description, W/2, 550, 780, 36);

    // Traits
    ctx.fillStyle='#d9d9e6'; ctx.font='600 19px "JetBrains Mono", monospace';
    ctx.fillText(arch.traits.join(' • ').toUpperCase(), W/2, afterDesc+34);

    // Best Pair line
    const compat=COMPATIBILITY[arch.key];
    if(compat){
        const pair=ARCHETYPES[compat.pair];
        ctx.fillStyle='rgba(168,85,247,0.9)'; ctx.font='700 16px "JetBrains Mono"'; ctx.fillText(`BEST PAIR: ${pair.name.toUpperCase()}`, W/2, afterDesc+74);
        ctx.fillStyle='rgba(255,255,255,0.65)'; ctx.font='400 15px "Space Grotesk"'; ctx.fillText(`"${compat.reason}"`, W/2, afterDesc+98);
    }

    // Footer line
    ctx.strokeStyle='rgba(255,255,255,0.09)'; ctx.beginPath(); ctx.moveTo(96,H-190); ctx.lineTo(W-96,H-190); ctx.stroke();

    // Feature 10: Generated on + QR
    const dateStr=new Date().toLocaleDateString('en-GB', {day:'2-digit', month:'short', year:'numeric'});
    ctx.fillStyle='rgba(255,255,255,0.42)'; ctx.font='500 16px "JetBrains Mono"'; ctx.textAlign='left';
    ctx.fillText(`Generated on ${dateStr}`, 96, H-132);
    ctx.fillStyle='rgba(255,255,255,0.40)'; ctx.font='500 14px "JetBrains Mono"';
    ctx.fillText(SITE_URL.replace('https://',''), 96, H-108);

    // QR Code bottom right with neon border
    try{
        ...
        // neon border
        ctx.save();
        ctx.shadowColor=arch.color; ctx.shadowBlur=22;
        ctx.strokeStyle=arch.color+'BB'; ctx.lineWidth=2;
        ctx.strokeRect(qrX-6, qrY-6, qrSize+12, qrSize+12);
        ctx.restore();
        ctx.drawImage(qrImg, qrX, qrY, qrSize, qrSize);
        ctx.fillStyle='rgba(255,255,255,0.55)'; ctx.font='600 10px "JetBrains Mono"'; ctx.textAlign='center';
        ctx.fillText('SCAN TO TRY', qrX+qrSize/2, qrY+qrSize+14);
    } catch(e){ console.warn('QR generation failed', e); }

    // Right side ID
    ctx.fillStyle='rgba(255,255,255,0.36)'; ctx.font='500 13px "JetBrains Mono"'; ctx.textAlign='right';
    ctx.fillText(`ID: ${arch.key.toUpperCase()}-${Math.floor(1000+Math.random()*9000)}-X`, W-96, H-108 - 24);

    ctx.textAlign='center'; ctx.fillStyle='rgba(255,255,255,0.28)'; ctx.font='400 13px "JetBrains Mono"';
    ctx.fillText('BUILT FOR THE FUTURE • NO COOKIES. JUST CODE.', W/2, H-70);
...
function showToast(){
    ...
    setTimeout(()=>{ DOM.toast.classList.remove('show'); DOM.toast.classList.add('hidden'); }, 2500);
    if(navigator.vibrate) navigator.vibrate(20);
}

// ============================================================================
// Hash Routing + Event Lock + Announcement
// ============================================================================
function hideAllSections(){
    [DOM.landing, DOM.quiz, DOM.result, DOM.locked, DOM.admin, DOM.uptime, DOM.notfound].forEach(s=>{
...
function showLocked(){
    hideAllSections();
    DOM.locked.style.display='block';
    void DOM.locked.offsetWidth;
    DOM.locked.classList.add('active');
}

function showUptime(){
    hideAllSections();
    DOM.uptime.style.display='block';
    void DOM.uptime.offsetWidth;
    DOM.uptime.classList.add('active');
    startUptimeTimer();
}

function show404(){
    hideAllSections();
    DOM.notfoundHash.textContent=location.hash || '#';
    DOM.notfound.style.display='block';
    void DOM.notfound.offsetWidth;
    DOM.notfound.classList.add('active');
}

function handleRouter(){
    const hash=location.hash;

    // Stop any reveal overlay
    DOM.revealSeq.classList.remove('active');

    if(hash==='#secret-admin-only'){
        hideAllSections();
        DOM.admin.style.display='flex';
        openAdminPanel();
        return;
    }
    if(hash==='#uptime-ping'){
    ...
    // Known internal hashes we ignore (quiz/result handled via JS)
    if(['#quiz','#result'].includes(hash)){
        // if user manually goes there, redirect to landing unless result already shown
        if(DOM.result.classList.contains('active') || DOM.quiz.classList.contains('active')){
            return;
        }
        showLanding();
        return;
...
function setupAnnouncementListener(){
    ...
    settingsUnsub=subscribeToSettings((settings)=>{
        ...
        // Banner
        if(settings.announcementVisible && settings.announcement){
        ...
        // Event live check: if currently on landing and event changed to closed, show locked
        if(!settings.eventLive){
            // if user is on landing/quiz/result, switch to locked unless admin route
            if(!location.hash.includes('secret-admin-only') && !location.hash.includes('uptime-ping') && !location.hash.includes('404')){
                if(DOM.landing.classList.contains('active') || DOM.quiz.classList.contains('active')){
                    showLocked();
                }
            }
        } else {
            // if event reopened and user is on locked screen, return to landing
            if(DOM.locked.classList.contains('active')){
                showLanding();
            }
        }
    });
...
function startUptimeTimer(){
    ...
    // Auto ping firebase every 30s
    setInterval(()=>{ /* keeps page alive */ if(isFirebaseConfigured()){ /* noop */ } },30000);
}
...
// ============================================================================
async function init(){
    console.log('%c🧬 DevDNA v2 Initialized', 'color:#a855f7; font-size:16px; font-weight:bold;');
    initSound();
    bindHoverSFX();
    initParticles();
    initMatrixRain();
    ...
    // Ensure all sections hidden initially except boot
    hideAllSections();
    DOM.bootScreen.style.display='flex';

    // Settings listener for banner + event
    ...
    handleRouter(); // initial

    // If no hash, show landing + typing
    ...
    window.__DevDNA={
        scores, ARCHETYPES, QUIZ_QUESTIONS, startQuiz, playSFX,
        SITE_URL
    };

    // Pre-warm leaderboard for total counter
    setupLeaderboard();
}
